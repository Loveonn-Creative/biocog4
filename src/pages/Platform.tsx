...
import { ArrowRight, FileCheck, Shield, Banknote, Target, Coins, Building2, Landmark, Globe2, Sparkles } from "lucide-react";
...
const outcomes = [
  { icon: FileCheck, title: "Report", body: "One verified ledger serves CBAM, BRSR, GHG Protocol, ISSB, CSRD, TCFD, GRI and SBTi without re-keying.", to: "/trust", cta: "How verification works" },
  { icon: Banknote, title: "Finance", body: "Verified baselines and a Climate Credibility Score become underwriting inputs for green loans and factoring.", to: "/climate-finance", cta: "Climate finance flow" },
  { icon: Target, title: "Decarbonize", body: "A baseline becomes a sector-aware roadmap. Reductions are tracked against the original methodology, not re-baselined.", to: "/net-zero", cta: "Net-Zero engine" },
  { icon: Coins, title: "Monetize", body: "Records that clear additionality, evidence linkage and methodology lock can become credit-ready signals for buyers.", to: "/carbon-credits", cta: "Carbon credits" },
];

const steps = [
  { n: "01", title: "Capture", body: "Drop a document, take a photo, or speak the data point. The first response arrives in under two seconds." },
  { n: "02", title: "Verify", body: "Every document is hashed, parsed, classified deterministically, and reconciled against country grid factors." },
  { n: "03", title: "Use", body: "The verified record feeds your disclosure, your lender, your reduction roadmap, and — when eligible — your credits." },
];
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="flex-1">
        {/* Hero */}
        <section className="container max-w-4xl mx-auto px-6 pt-32 pb-20 text-center">
        ...
        <section className="border-t border-border py-20">
          <div className="container max-w-3xl mx-auto px-6">
            <div className="text-xs uppercase tracking-wider text-muted-foreground mb-3">{t("platform.what.eyebrow", "What is Senseible?")}</div>
        ...
        <section className="border-t border-border py-20 bg-secondary/30">
          <div className="container max-w-5xl mx-auto px-6">
            <div className="text-xs uppercase tracking-wider text-muted-foreground mb-3">{t("platform.who.eyebrow", "Who is it for?")}</div>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {audiences.map(a => (
                <Card key={a.key} className="border-border">
                  <CardContent className="p-6">
                    <div className="text-sm font-medium mb-2">{t(`platform.who.${a.key}.label`, a.label)}</div>
        ...
        <section className="border-t border-border py-20">
          <div className="container max-w-3xl mx-auto px-6">
            <div className="text-xs uppercase tracking-wider text-muted-foreground mb-3">{t("platform.problem.eyebrow", "What problem does it solve?")}</div>
        ...
        <section className="border-t border-border py-20 bg-secondary/30">
          <div className="container max-w-5xl mx-auto px-6">
            <div className="text-xs uppercase tracking-wider text-muted-foreground mb-3">{t("platform.how.eyebrow", "How it works")}</div>
              ...
              {steps.map(s => (
                <Card key={s.n} className="border-border">
                  <CardContent className="p-6">
                    ...
                    <div className="text-lg font-medium mb-2">{t(`platform.how.${s.n}.title`, s.title)}</div>
                    <p className="text-sm text-muted-foreground leading-relaxed">{t(`platform.how.${s.n}.body`, s.body)}</p>
                  </CardContent>
                </Card>
              ))}
            </div>
        ...
        <section className="border-t border-border py-20">
          <div className="container max-w-5xl mx-auto px-6">
            <div className="text-xs uppercase tracking-wider text-muted-foreground mb-3">{t("platform.out.eyebrow", "What you can do with verified data")}</div>
                  ...
                  <CardContent className="p-6">
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="text-base font-medium mb-2">{t(`platform.out.${o.title}.title`, o.title)}</div>
                        <p className="text-sm text-muted-foreground leading-relaxed mb-3">{t(`platform.out.${o.title}.body`, o.body)}</p>
                        <Link to={o.to} className="text-sm text-primary hover:underline inline-flex items-center gap-1">
                          {t(`platform.out.${o.title}.cta`, o.cta)} <ArrowRight className="h-3.5 w-3.5" />
                        </Link>
                      </div>
                    </div>
                  </CardContent>
        ...
        <section className="border-t border-border py-20">
          <div className="container max-w-5xl mx-auto px-6">
            <div className="flex items-center gap-3 mb-3">
              ...
              <span className="text-xs uppercase tracking-wider text-muted-foreground">{t("platform.industries.eyebrow", "Industries")}</span>
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              {["textile", "steel", "chemical", "logistics", "automobile", "construction"].map(s => (
                <Link key={s} to={`/industries/${s}`} className="px-4 py-2 rounded-full border border-border text-sm text-foreground/80 hover:bg-secondary transition-colors capitalize">
                  {s}
                </Link>
              ))}
            </div>
        ...
        <section className="border-t border-border py-20 bg-secondary/30">
          <div className="container max-w-5xl mx-auto px-6">
            <div className="flex items-center gap-3 mb-3">
              ...
              <span className="text-xs uppercase tracking-wider text-muted-foreground">{t("platform.where.eyebrow", "Where it works")}</span>
            </div>
